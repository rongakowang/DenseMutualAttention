...
class ResNetBackbone(nn.Module):
    ...
    def __init__(self, resnet_type, input_dim=3):
    ...
    def forward(self, x, skip_early=False):
        ...
        if not skip_early:
            return x1, x5, [x2, x3, x4], x0
        else:
            return x1, x5, [x2, x3, x4]

    def init_weights(self, skip_first=False):
        org_resnet = torch.utils.model_zoo.load_url(model_urls[self.name], map_location='cpu')
