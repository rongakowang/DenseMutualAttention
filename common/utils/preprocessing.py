...
def get_aug_config(exclude_flip):
    ...
    # rot = np.clip(np.random.uniform(), -3.0,
    #               3.0) * rot_factor if random.random() <= 0.6 else 0
...
def crop_image(img, obj_mask, bbox, cam_param):
    scale, rot, do_flip = 1.0, 0.0, False
    img, trans, inv_trans = generate_patch_image(img, bbox, scale, rot, do_flip, cfg.input_img_shape)
    obj_mask, _, _ = generate_patch_image(obj_mask, bbox, scale, rot, do_flip, (256, 256))
    cam_param_aug = trans @ cam_param
    cam_param_aug = np.concatenate([cam_param_aug, np.array([[0., 0., 1.]])], axis=0)
    return img, obj_mask, cam_param_aug

def augmentation(img, mask, tex_mask, obj_mask, bbox, data_split, cam_param, exclude_flip=False):
    if data_split == 'train':
        scale, rot, color_scale, do_flip = get_aug_config(exclude_flip)
        brightness = random.uniform(0.8, 1.2)
        contrast = random.uniform(0.8, 1.2)
        if not cfg.scale_change:
            scale = 1.0
        if not cfg.rot_change:
            rot = 0.0
    else:
        scale, rot, color_scale, do_flip = 1.0, 0.0, np.array([1, 1, 1]), False
        brightness = 1
    ...
    assert cfg.input_img_shape == (256, 256)
    # disable augmentation
    if not cfg.bbox_crop:
        bbox = [0, 0, 256, 256]
    img, trans, inv_trans = generate_patch_image(img, bbox, scale, rot, do_flip, cfg.input_img_shape)
    mask, _, _ = generate_patch_image(mask, bbox, scale, rot, do_flip, (256, 256))
    tex_mask, _, _ = generate_patch_image(tex_mask, bbox, scale, rot, do_flip, (256, 256))
    obj_mask, _, _ = generate_patch_image(obj_mask, bbox, scale, rot, do_flip, (256, 256))

    # modify camera_param, if no augmentation, then this should be the same as cam_param
    cam_param_aug = trans @ cam_param
    cam_param_aug = np.concatenate([cam_param_aug, np.array([[0., 0., 1.]])], axis=0)
    ...
    img = np.clip(img * color_scale[None, None, :], 0, 255)
    return img, mask, tex_mask, obj_mask, trans, inv_trans, rot, cam_param_aug, do_flip, scale
